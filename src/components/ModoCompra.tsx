...
export const ModoCompra: React.FC<{ context: AppContextType}> = ({ context}) => {
 ...
 {/* STICKY HEADER */}
 <div className={`sticky top-0 z-30 pt-[calc(env(safe-area-inset-top)+20px)] px-5 pb-6 rounded-b-[40px] geometric-bg shadow-lg transition-colors duration-500 ${headerColor} ${pulseClass}`}>
...
>
 ...
 <div>
 <div className={`text-[12px] font-semibold mb-1 ${subTextColor}`}>Valor no Carrinho</div>
 ...
 <span className={`money-value transition-transform duration-150 inline-block ${scaleTotal ? 'scale-[1.06]' : 'scale-100'}`}>{totalSpent.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
 </div>
...
>
 ...
 <button onClick={handleAvulsoAdd} className="bg-emerald-600 text-white px-5 py-3 rounded-xl font-semibold text-sm shadow-md active:scale-[0.97] transition-transform duration-150">Adicionar</button>
 </div>
 )}
 ...
 <h3 className="text-[14px] font-bold text-zinc-400 dark:text-zinc-500 whitespace-nowrap">{CATEGORY_EMOJI_UPDATED[category] || '🛒'} {category}</h3>
...
>
 ...
 <span className="text-[10px] font-semibold text-zinc-400 pr-1">{item.unit}</span>
 </div>
 ...
 <span className="money-value">{formatMoney(precoUnitario)}</span><span className={`text-[9px] font-medium ${isCurrentMarket ? 'text-emerald-400' : 'text-zinc-400'}`}>/{item.unit}</span>
 </div>
 </div>
 );
})
