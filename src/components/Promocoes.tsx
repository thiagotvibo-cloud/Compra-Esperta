...
export const Promocoes: React.FC<{ context: AppContextType}> = ({ context}) => {
 ...
 {/* HEADER */}
 <div className="bg-gradient-to-br from-emerald-500 to-teal-400 rounded-b-[40px] geometric-bg pt-[calc(env(safe-area-inset-top)+32px)] pb-20 px-6 text-white shadow-primary z-10 relative">
 ...
 <Store size={18} className="text-emerald-500"/> Selecione o Mercado
 </label>
 ...
 className="flex-1 px-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl font-semibold text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-inner appearance-none"
>
 ...
 {selectedMarket && (
 <button onClick={() => removeMarket(selectedMarket)} className="p-3.5 text-zinc-400 bg-zinc-100 dark:bg-zinc-800 border-none hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-3xl transition-colors">
 <Trash2 size={22} />
 </button>
 )}
 </div>
 ...
 className="flex-1 min-w-0 px-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl focus:outline-none focus:ring-2 focus:ring-emerald-500 placeholder-zinc-400 font-medium"
 />
 <button type="submit"className="shrink-0 bg-emerald-600 hover:bg-emerald-700 transition-colors text-white px-5 py-3.5 rounded-3xl font-semibold active:scale-[0.97] transition-transform duration-150 shadow-sm">Criar</button>
 </form>
 </div>
 ...
 <h3 className="text-[13px] font-bold text-emerald-600 dark:text-emerald-500 mb-4 flex items-center gap-2">
 <Plus size={18} strokeWidth={3} /> {editingPromoId ? 'Editar Oferta' : 'Adicionar Nova Oferta'}
 </h3>
 <form onSubmit={handleAddPromotion} className="space-y-4">
 <div>
 <label className="block text-[11px] font-semibold mb-1.5 text-zinc-500">Produto</label>
 <div 
 onClick={() => setShowCatalog(true)}
 className={`w-full px-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl cursor-pointer font-semibold ${itemName ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-400'}`}
>
 ...
 <div>
 <label className="block text-[11px] font-semibold mb-1.5 text-zinc-500">Preço (R$)</label>
 <div className="relative">
 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[12px] font-semibold text-zinc-400">R$</span>
 <input type="tel"value={getPriceDisplayValue(price)} onChange={e => handlePriceInput(e.target.value)} placeholder="0,00"className="w-full pl-9 pr-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl font-bold text-[16px] text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"required />
 </div>
 </div>
 <div>
 <label className="block text-[11px] font-semibold mb-1.5 text-zinc-500">Por (Qtd / Un)</label>
 <div className="flex gap-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-3xl p-1.5 focus-within:ring-2 focus-within:ring-emerald-500">
 ...
 <select value={unit} onChange={e => setUnit(e.target.value as Unit)} className="w-1/2 px-1 py-2 bg-transparent border-none outline-none focus:outline-none font-semibold text-zinc-500 appearance-none"style={{ WebkitAppearance: 'none', MozAppearance: 'none'}}>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="block text-[11px] font-semibold mb-1.5 text-zinc-500">Anotação</label>
 <input type="text"value={notes} onChange={e => setNotes(e.target.value)} placeholder="Ex: Marca Ype..."className="w-full px-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium placeholder-zinc-400"/>
 </div>
 <div>
 <label className="block text-[11px] font-semibold mb-1.5 text-zinc-500">Validade</label>
 <input type="date"value={expiryDate} onChange={e => setExpiryDate(e.target.value)} className="w-full px-4 py-3.5 bg-zinc-100 dark:bg-zinc-800 border-none rounded-3xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium text-zinc-900 dark:text-zinc-100"/>
 </div>
 </div>

 <div className="flex flex-col sm:flex-row gap-2 mt-2 pt-2">
 <button type="submit"className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-3xl font-bold text-[15px] transition-transform active:scale-[0.97] transition-transform duration-150 shadow-sm">
 {editingPromoId ? 'Salvar Alterações' : 'Salvar Oferta'}
 </button>
 {editingPromoId && (
 <button type="button"onClick={cancelEdit} className="w-full sm:w-auto px-6 py-4 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-3xl font-semibold transition-colors active:scale-[0.97] transition-transform duration-150">
 Cancelar
 </button>
 )}
 </div>
 ...
 className={`px-4 py-2 rounded-xl font-bold text-[12px] transition-colors shrink-0 ${promoFilter === 'all' ? 'bg-emerald-500 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}
>
 Todas Ofertas
 </button>
 <button 
 onClick={() => setPromoFilter('today')} 
 className={`px-4 py-2 rounded-xl font-bold text-[12px] transition-colors shrink-0 ${promoFilter === 'today' ? 'bg-red-500 text-white' : 'bg-red-50 dark:bg-red-900/10 text-red-500'}`}
>
 Vence Hoje
 </button>
 <button 
 onClick={() => setPromoFilter('tomorrow')} 
 className={`px-4 py-2 rounded-xl font-bold text-[12px] transition-colors shrink-0 ${promoFilter === 'tomorrow' ? 'bg-orange-500 text-white' : 'bg-orange-50 dark:bg-orange-900/10 text-orange-500'}`}
>
 Vence Amanhã
 </button>
 </div>
 ...
 return (
 <div key={promo.id} className="bg-white dark:bg-zinc-900 p-4 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 flex justify-between items-center gap-4 transition-all cursor-pointer active:scale-[0.97] transition-transform duration-150"onClick={() => handleEditPromo(promo)}>
 <div className="flex-1 min-w-0 pointer-events-none">
 
 <div className="flex items-center gap-2 mb-1">
 <span className="text-[10px] font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded-full flex items-center gap-1"><Store size={10} /> {marketName}</span>
 </div>
 ...
 <div className="text-emerald-600 dark:text-emerald-400 font-bold text-[22px] tracking-tight leading-none"><span className="money-value">{formatMoney(promo.price)}</span></div>
 </div>
 ...
 Equivale <span className="money-value">{formatMoney(pricePerBase)}</span> / {base.unit}
 </span>
 )}
 </div>
...
>
 ...
 className="w-full pl-10 pr-4 py-3.5 bg-white dark:bg-[#1C1C1E] border border-zinc-200 dark:border-zinc-800 rounded-3xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-semibold text-[15px] dark:text-zinc-100 transition-colors shadow-sm placeholder-zinc-400"
 />
 {searchQuery && (
 ...
 <h4 className="text-[11px] font-semibold text-zinc-400 mb-4">Resultados</h4>
 <div className="flex flex-wrap gap-2">
 {searchResults.map((item, index) => (
 <button
 key={index}
 onClick={() => handleAddFromCatalog(item.name)}
 className="px-4 py-2.5 bg-zinc-100 hover:bg-emerald-50 dark:bg-zinc-800 dark:hover:bg-emerald-900/30 text-zinc-800 dark:text-zinc-200 hover:text-emerald-700 dark:hover:text-emerald-400 text-[14px] font-semibold rounded-3xl transition-colors flex items-center gap-1.5 active:scale-[0.97] transition-transform duration-150"
>
...
>
 ...
 <div key={j}>
 <h4 className="text-[11px] font-semibold text-zinc-400 mb-3">{sub.name}</h4>
 <div className="flex flex-wrap gap-2">
