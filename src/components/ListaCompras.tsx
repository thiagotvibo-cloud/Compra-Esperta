...
export const ListaCompras: React.FC<{ context: AppContextType}> = ({ context}) => {
 ...
 {/* HEADER MARKET PRO */}
 <div className={`bg-gradient-to-br from-emerald-500 to-teal-400 rounded-b-[40px] geometric-bg px-6 text-center text-white shadow-primary z-10 transition-all duration-300 ${scrolled ? 'pt-[calc(env(safe-area-inset-top)+8px)] pb-3' : 'pt-[calc(env(safe-area-inset-top)+20px)] pb-14'}`}>
 ...
 <p className="text-emerald-50 font-semibold text-[11px] mb-1.5">Orçamento Planejado</p>
 <h1 className="text-[44px] font-bold tracking-tight leading-none mb-3">
 <span className="money-value">{formatMoney(settings.budget)}</span>
 </h1>
 ...
 <span>Total Estimado: <span className="money-value">{formatMoney(expectedTotal)}</span></span>
 ...
 <span className="bg-green-500/20 text-green-100 px-1.5 py-0.5 rounded-md">Se comprar onde tem oferta, você poupará <span className="money-value">{formatMoney(calculateEconomy())}</span>.</span>
 </div>
 )}
 </div>
 )}
 ...
 <div className="w-14 h-14 rounded-full bg-emerald-50 text-emerald-500 dark:bg-emerald-900/30 dark:text-emerald-400 flex items-center justify-center transition-transform hover:scale-105 active:scale-[0.97] transition-transform duration-150 shadow-sm border border-emerald-100 dark:border-emerald-800/30">
 ...
 <div className="w-14 h-14 rounded-full bg-blue-50 text-blue-500 dark:bg-blue-900/30 dark:text-blue-400 flex items-center justify-center transition-transform hover:scale-105 active:scale-[0.97] transition-transform duration-150 shadow-sm border border-blue-100 dark:border-blue-800/30">
 ...
 <div className="w-14 h-14 rounded-full bg-zinc-50 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 flex items-center justify-center transition-transform hover:scale-105 active:scale-[0.97] transition-transform duration-150 shadow-sm border border-zinc-100 dark:border-zinc-700/50">
...
>
 ...
 <div>
 <h4 className="text-[12px] font-semibold text-emerald-700 dark:text-emerald-400 mb-1">Dica da IA</h4>
...
>
 ...
 className="mb-6"
>
 <motion.div layout className="text-[13px] font-bold text-zinc-400 dark:text-zinc-500 mt-2 mb-3 pl-1">
...
>
 ...
 <span>Melhor: <span className="money-value">{formatMoney(promo.price)}</span> {market ? `no ${market.name}` : ''}</span>
 </div>
 );
})()}
 </div>
 ...
 className="bg-transparent text-[12px] font-semibold text-zinc-500 pr-0.5 focus:outline-none cursor-pointer appearance-none"
...
>
 ...
 className="w-full py-3.5 mt-2 rounded-3xl font-bold text-zinc-600 bg-zinc-100 dark:bg-zinc-800 dark:text-zinc-300 active:scale-[0.97] transition-transform duration-150 transition-all"
>
 Cancelar
 </button>
 </div>
...
>
 ...
 <h4 className="text-[12px] font-semibold text-soft-text-muted mb-3">{sub.name}</h4>
 <div className="flex flex-wrap gap-2">
