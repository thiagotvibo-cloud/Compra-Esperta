        ...
        <AnimatePresence>
          {tip && (
            <motion.div 
               ...
               className="bg-zinc-200 dark:bg-zinc-800 p-4 rounded-2xl border border-zinc-300 dark:border-zinc-700 flex items-start gap-3"
            >
              <Lightbulb className="text-zinc-400 dark:text-zinc-500 shrink-0 mt-0.5" size={20} />
              <div>
                <h4 className="text-[12px] font-semibold text-zinc-800 dark:text-zinc-200 mb-1 uppercase tracking-wider">Dica da IA</h4>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-[110] flex justify-center items-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in" onClick={() => setShowClearConfirm(false)}>
          <div className="bg-soft-bg dark:bg-zinc-900 rounded-3xl p-6 w-full max-w-sm shadow-xl text-center" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-xl font-bold text-zinc-800 dark:text-zinc-200 mb-2">Opções de Limpeza</h3>
            ...
            <div className="flex flex-col gap-3">
               <button 
                 ...
                 className="w-full py-3.5 rounded-2xl font-bold text-zinc-900 dark:text-white bg-orange-500 hover:bg-orange-600 active:scale-95 transition-all shadow-sm"
               >
                 Desmarcar Itens (Limpar Carrinho)
               </button>
               <button 
                 onClick={() => { setItems([]); setShowClearConfirm(false); }} 
                 className="w-full py-3.5 rounded-2xl font-bold text-zinc-900 dark:text-white bg-red-500 hover:bg-red-600 active:scale-95 transition-all shadow-sm"
               >
                ...
                <>
                  ...
                  return (
                    <div key={i} className="bg-soft-card dark:bg-zinc-800 rounded-[24px] border-none overflow-hidden transition-all shadow-sm">
                      ...
                      >
                        <span className="font-semibold text-soft-text-main dark:text-zinc-200 text-[15px] flex items-center gap-3">
