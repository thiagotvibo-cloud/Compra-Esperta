...
export const Dashboard: React.FC<{ context: AppContextType }> = ({ context }) => {
  ...
  const projectedEconomy = Math.max(0, simBudget - simulatedCost) * 0.1; // 10% of leftover as potential economy
  ...
  return (
    <div className="pb-28 bg-transparent min-h-screen">
      ...
      <div className="px-6 mt-4 relative z-20 pb-24 space-y-6">
        
        {/* CARDS PRINCIPAIS */}
        <section className="grid grid-cols-2 gap-4">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[24px] p-5 shadow-sm flex flex-col justify-between">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-2">
              <Wallet size={16} />
              <span className="text-[11px] font-bold uppercase tracking-wider">Disponível</span>
            </div>
            <div>
              <div className={`text-2xl font-bold ${remaining < 0 ? 'text-red-500' : 'text-slate-900 dark:text-slate-100'}`}>
                {formatMoney(remaining)}
              </div>
              <div className="text-[12px] font-medium text-slate-500 mt-1">
                de {formatMoney(settings.budget || 0)}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[24px] p-5 shadow-sm flex flex-col justify-between">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-2">
              ...
              <span className="text-[11px] font-bold uppercase tracking-wider">Gastos Mês</span>
            </div>
            <div>
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                {formatMoney(spentThisMonth)}
              </div>
              <div className="text-[12px] font-medium text-green-600 dark:text-green-500 mt-1 flex items-center gap-1">
                <TrendingUp size={12} /> {formatMoney(economyThisMonth)} economizados
              </div>
            </div>
          </div>
        </section>
        ...
        {/* PROGRESSO E ALERTAS */}
        <section className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[24px] p-5 shadow-sm">
          ...
          {isDanger ? (
            <div className="flex gap-3 items-start bg-red-50 dark:bg-red-950/20 p-3 rounded-2xl border border-red-100 dark:border-red-900/30">
            ...
            <div className="flex gap-3 items-start bg-amber-50 dark:bg-amber-950/20 p-3 rounded-2xl border border-amber-100 dark:border-amber-900/30">
            ...
            <div className="flex gap-3 items-start bg-green-50 dark:bg-green-950/20 p-3 rounded-2xl border border-green-100 dark:border-green-900/30">
        ...
        {/* PRÓXIMO PASSO */}
        <section className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-zinc-800 dark:to-zinc-900 rounded-[24px] p-5 text-white shadow-lg relative overflow-hidden">
        ...
        <section className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[24px] shadow-sm overflow-hidden">
          ...
          <div className="p-5 space-y-5">
            ...
            <div className={`p-5 rounded-3xl border ${simulatedIsOk ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-100 dark:border-blue-900/30' : 'bg-red-50 dark:bg-red-950/20 border-red-100 dark:border-red-900/30'}`}>
            ...
            {simBudget !== (settings.budget || 500) && (
              <button 
                ...
                className="w-full py-3.5 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-2xl font-bold text-[14px] shadow-sm hover:scale-[1.02] active:scale-95 transition-all"
              >
