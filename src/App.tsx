...
export default function App() {
 ...
 // Ignore missing table errors silently, as it means the user hasn't run the SQL setup script yet
 if (error?.code === '42P01' || error?.code === 'PGRST205' || msg.includes('schema cache')) {
 return;
}
 ...
 const { error: insErr} = await supabase.from('settings').upsert({ budget: 0, dark_mode: false, streak: 0, total_saved: 0, last_active_date: new Date().toISOString().split('T')[0], purchase_count: 0, user_id: session.user.id}, { onConflict: 'user_id'});
 ...
 if (hErr && hErr.code !== '42P01') handleError('Buscar Histórico', hErr); // 42P01/PGRST205 is table undefined, handle gracefully if user hasn't created it yet
 ...
 if (selectErr && selectErr.code !== '42P01') return handleError('Listar Histórico para Sincronização', selectErr);

 const idsToDelete = existingIds?.map(e => e.id).filter(id => !currentIds.includes(id)) || [];
 
 if (validHistoryToUpsert.length> 0) {
 const { error: upErr} = await supabase.from('history').upsert(validHistoryToUpsert);
 if (upErr && upErr.code !== '42P01') handleError('Salvar/Atualizar Histórico', upErr);
}
 if (idsToDelete.length> 0) {
 const { error: delErr} = await supabase.from('history').delete().in('id', idsToDelete);
 if (delErr && delErr.code !== '42P01') handleError('Excluir Histórico', delErr);
}
} catch (err) {
 ...
 <button onClick={handleInstallClick} className="bg-white text-emerald-600 px-3 py-1 rounded-full text-sm font-bold shadow-sm active:scale-[0.97] transition-transform duration-150 transition-transform">Instalar</button>
 </div>
 </div>
 )}
...
>
 ...
 className="w-1.5 h-1.5 rounded-full bg-emerald-500 absolute -bottom-1.5"
